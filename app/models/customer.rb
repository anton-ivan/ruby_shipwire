class Customer < ActiveRecord::Base
  ...
  has_many :recurrent_orders, as: :orderer, dependent: :destroy
  
  validates :first_name, presence: true
  validates :last_name, presence: true
  validates :email, presence: true, format: { with: /\A([^@\s]+)@((?:[-a-z0-9]+\.)+[a-z]{2,})\Z/i }
  validates :address1, :presence => { :message => "Please enter address" }
  validates :city, presence: true
  validates :country, :presence => { :message => "Please select county" }
  validates :state, :presence => { :message => "Please select state" }
